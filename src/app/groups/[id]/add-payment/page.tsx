...
import { Participant } from '@/types/payment'
...
export default function AddPaymentPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && groupId) {
      ...
      if (groupData && groupData.members.length > 0) {
        setPayerId(groupData.members[0].id)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!paymentManager || !group) return
    ...
    try {
      paymentManager.createPayment(groupId, amount, description, payerId, participants)
      ...
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 py-6">
          <div>
            ...
            <h1 className="text-3xl font-bold text-gray-900 mt-2">支払いを追加</h1>
          </div>
        </div>
      </header>
